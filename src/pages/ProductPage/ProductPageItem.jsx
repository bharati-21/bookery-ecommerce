        ...
        if(isAuth) {
            if(bookInWishList) {
                ...
                if(prodcutDeletedFromWishList) {   
                    ...
                    showToast('Failed to remove item from wishlist. Please try again later.', 'error');
                }
            }
            else {
                const productPostedToWishList = await postToWishList(book, token);
    
                if(productPostedToWishList) {   
                    showToast('Item added to wishlist', 'success');
                    wishListDispatch({type: 'ADD_TO_WISHLIST', payload: {wishListItems: productPostedToWishList, error:false, loading: false}});
                    setBookInWishList(true);
                }
                else {
