...
const BookTypeFilterOption = ({ bookTypeFilter }) => {
    ...
    return (
        <li className="list-item mt-0-75 mb-0-5">
            <label htmlFor={`cover-${bookTypeFilter}`} className="text-sm book-cover-label flex-row flex-align-center">
                <input type="checkbox" value={bookTypeFilter} className="input-checkbox" id={`cover-${bookTypeFilter}`} onChange={handleBookTypeChange} checked={isBookTypeChecked} />
