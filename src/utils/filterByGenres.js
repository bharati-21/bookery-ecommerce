const filterByGenres = (books, genreFilters) => {
    ...
    const areNoGenresSelected = Object.values(genreFilters).every(genreIsChecked => !genreIsChecked);
    ...
    for(const genre in genreFilters) {
        ...
        const booksWithGenre = books.filter(({ id, genres: bookGenres }) => isGenreChecked && !filteredBooks.find(filteredBook => filteredBook.id === id) ? bookGenres.includes(genre) : false);
